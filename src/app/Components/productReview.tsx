...
export default function Reviews() {
  ...
  const [visibleReviews, setVisibleReviews] = useState(initialReviews.slice(0, 4)); // Initially show only 4 reviews
  ...
  return (
    <div className="bg-white min-h-screen">
      ...
      <header className="border-b">
        <nav className="flex underline font-extrabold justify-evenly space-x-8 py-4">
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        <div className=" grid grid-cols-1 sm:grid-cols-2 space-y-2 justify-center items-center mb-6">
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {visibleReviews.map((review, index) => (
            <div key={index} className=" border-2 rounded-lg p-4 shadow-sm">
              ...
              <div className="mb-4">
                ...
                <div className="flex space-x-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span
                      key={star}
                      className={`cursor-pointer ${star <= newReview.rating ? "text-blue-500" : "text-gray-300"}`}
